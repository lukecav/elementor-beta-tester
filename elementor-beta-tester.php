...
/**
 * Confirm Elementor is at least installed before doing anything
 ...
 */
if ( ! file_exists( trailingslashit( dirname( dirname( __FILE__ ) ) ) . 'elementor/elementor.php' ) ) :

	add_action( 'admin_notices', 'embt_elementor_not_installed' );

elseif ( ! class_exists( 'EM_Beta_Tester' ) ) :

	/**
	 * EM_Beta_Tester Main Class
	 */
	class EM_Beta_Tester {
		...
		public static function activate() {
			...
			delete_site_transient( 'elementor_latest_tag' );
		}
		...
		public function set_update_args() {
			...
			$this->config[ 'zip_url' ]      = 'https://github.com/pojome/elementor/zipball/' . $this->config[ 'new_version' ];
		}
		...
		public function overrule_transients() {
			return ( defined( 'EM_BETA_TESTER_FORCE_UPDATE' ) && EM_BETA_TESTER_FORCE_UPDATE );
		}
		...
		public function upgrader_source_selection( $source, $remote_source, $upgrader ) {
			...
			if ( strstr( $source, '/elementor-master-' ) ) {
	...
	register_activation_hook( __FILE__, array( 'EM_Beta_Tester', 'activate' ) );

	add_action( 'admin_init', array( 'EM_Beta_Tester', 'instance' ) );
...
**/
if ( ! function_exists( 'embt_elementor_not_installed' ) ) {

	function embt_elementor_not_installed() {

		echo '<div class="error"><p>' . sprintf( __( 'Elementor Beta Tester requires %s to be installed.', 'elementor-beta-tester' ), '<a href="https://elementor.com/" target="_blank">Elementor</a>' ) . '</p></div>';
